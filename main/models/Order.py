...
class Order(JsonMixin, TimestampMixin, db.Model):
    ...
    item_id = db.Column('item_id', db.Integer, db.ForeignKey(
        'restaurants.id'), nullable=False)
    user = db.relationship('User', backref=db.backref(
        'orders', lazy='dynamic'), lazy='joined')
    item = db.relationship('Restaurant', backref=db.backref(
        'orders', lazy='dynamic'), lazy='joined')
    ...
    public_keys = ['user', 'item', 'created_at', 'updated_at', 'id',
                   'code', 'ordered_at', 'order_place', 'amount', 'note', 'status']
